...
log.info("Field Name,Field Id, Field Name ")
          ...
          tabs.each() {FieldScreenTab fst ->
            ...
            fsli.each() {it ->
                  ...
                  if (it.getFieldId().contains('customfield_')) {
                        if (uniqueFields(customFieldList, it.getFieldId())) {
                           
                        CustomField cf = fieldMgr.getCustomField(it.getFieldId())
                        log.info( cf.fieldName + "," + cf.customFieldType.descriptor )
                        }
...
def boolean  uniqueFields(Map map,String field) {
   boolean retval = true
